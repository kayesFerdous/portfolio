...
function FeatureCard({ title, description, buttonText, children, className }: FeatureCardProps) {
  ...
  return (
    <div
      className={cn(
        "relative flex flex-col justify-between p-8 min-h-[600px] border-r border-border bg-background overflow-hidden group",
        className
      )}
    ...
    >
      ...
      <div className="relative z-10">
        <h3 className="text-3xl font-bold text-foreground dark:text-muted-foreground group-hover:text-muted-foreground dark:group-hover:text-foreground transition-colors mb-4">{title}</h3>
      ...
      <div className="relative z-10 mt-auto">
        <button className="flex items-center gap-2 px-6 py-2 rounded-full border border-border text-muted-foreground text-sm font-mono uppercase tracking-wider hover:bg-foreground hover:text-background transition-colors">
  ...
  return (
    <section className="w-full bg-background py-20">
      <div className="container mx-auto px-4">
        <div className="grid grid-cols-1 md:grid-cols-3 border-l border-t border-b border-border">
          {/* Card 1: Grok Style */}
          <FeatureCard
            title="Grok"
          ...
          >
            <div className="relative w-64 h-64 opacity-20">
              <svg viewBox="0 0 100 100" className="w-full h-full stroke-foreground fill-none stroke-[0.5]">
          ...
          >
            <div className="relative w-full max-w-[300px] aspect-video border border-border bg-muted/50 rounded-lg p-2 transform rotate-y-12 rotate-x-6 shadow-2xl">
          ...
          >
            <div className="relative w-64 h-80">
              {[0, 1, 2].map((i) => (
                <div
                  key={i}
                  className="absolute inset-0 border border-border bg-muted/80 rounded-lg"
                  style={{
                    top: i * 10,
                    left: i * 10,
                    zIndex: 3 - i,
